...
Threshold: 999
Title: Day 1 Tornado Probability - $Day1TextValidTime
Color: 222 62 9
";
...
for ($i=0; $i <= $TorCount; $i++) {
    $RGB = prob_color('Tor', $TorProb[$i], 60); // Opacity Number
    ...
    $OutlookAreaShaded = PF_Polygon($ProbabilityRing, $Tors[$i], $RGBA);
    $PlacefileText .= "\n$OutlookAreaShaded\n";

    $RGBOnly = implode(" ", array_slice(explode(" ", $RGB), 0, 3));
    $OutlookAreaOutline = PF_LineBox($ProbabilityRing, $Tors[$i], $RGBOnly);
    $PlacefileText .= "\n$OutlookAreaOutline\n";
}
