...
Threshold: 999
Title: Day 1 Hail Probability - $Day1TextValidTime
Color: 222 62 9
";
...
for ($i=0; $i <= $HailCount; $i++) {
    $RGB = prob_color('Hail', $HailProb[$i], 60); // Opacity Number
    ...
    $OutlookAreaShaded = PF_Polygon($ProbabilityRing, $Hails[$i], $RGBA);
    $PlacefileText .= "\n$OutlookAreaShaded\n";

    $RGBOnly = implode(" ", array_slice(explode(" ", $RGB), 0, 3));
    $OutlookAreaOutline = PF_LineBox($ProbabilityRing, $Hails[$i], $RGBOnly);
    $PlacefileText .= "\n$OutlookAreaOutline\n";
}
