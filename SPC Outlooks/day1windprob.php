...
Threshold: 999
Title: Day 1 Wind Probabilities - $Day1TextValidTime
Color: 222 62 9
";
...
for ($i=0; $i <= $WindCount; $i++) {
    $RGB = prob_color('Wind', $WindProb[$i], 60); // Opacity Number
    ...
    $RGBA = str_replace(" ", ",", $RGB);
    
    $OutlookAreaShaded = PF_Polygon($ProbabilityRing, $Winds[$i], $RGBA);
    $PlacefileText .= "\n$OutlookAreaShaded\n";

    $RGBOnly = implode(" ", array_slice(explode(" ", $RGB), 0, 3));
    $OutlookAreaOutline = PF_LineBox($ProbabilityRing, $Winds[$i], $RGBOnly);
    $PlacefileText .= "\n$OutlookAreaOutline\n";
}
